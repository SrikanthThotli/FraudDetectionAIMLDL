...
class Components:
    ...
    def generate_transactions_table(self, customer_profile, start_date="2022-02-01", nb_days=10):
    ...
    def generate_dataset(self, n_customers=10000, n_terminals=1000000, nb_days=90, start_date="2022-02-01", r=5):
        ...
        customer_profiles_table['available_terminals'] = customer_profiles_table.apply(
            lambda x: self.get_list_terminals_within_radius(x, x_y_terminals=x_y_terminals, r=r), axis=1)
        # With Pandarallel customer_profiles_table['available_terminals'] = customer_profiles_table.parallel_apply(
        # lambda x : get_list_closest_terminals(x, x_y_terminals=x_y_terminals, r=r), axis=1)
        customer_profiles_table['nb_terminals'] = customer_profiles_table.available_terminals.apply(len)
        print("Time to associate terminals to customers: {0:.2}s".format(time.time() - start_time))

        start_time = time.time()
        transactions_df = customer_profiles_table.groupby('CUSTOMER_ID').apply(
            lambda x: self.generate_transactions_table(x.iloc[0], nb_days=nb_days)).reset_index(drop=True)
        # With Pandarallel transactions_df=customer_profiles_table.groupby('CUSTOMER_ID').parallel_apply(lambda x :
        # generate_transactions_table(x.iloc[0], nb_days=nb_days)).reset_index(drop=True)

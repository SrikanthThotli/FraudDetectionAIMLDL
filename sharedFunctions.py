...
class SharedMethod:
    ...
    def scaleData(self, train, test, features):
    ...
    def get_train_test_set(self, transactions_df,
        ...
        # If subsample
        if sampling_ratio < 1:
            train_df_frauds = train_df[train_df.TX_FRAUD == 1].sample(frac=sampling_ratio,
                                                                      random_state=random_state)
            train_df_genuine = train_df[train_df.TX_FRAUD == 0].sample(frac=sampling_ratio,
                                                                       random_state=random_state)
    ...
    def get_train_delay_test_set(self, transactions_df,
                                 start_date_training,
                                 delta_train=7, delta_delay=7, delta_test=7,
                                 sampling_ratio=1.0,
                                 random_state=0):
    ...
    def fit_model_and_get_predictions(self, classifier, train_df, test_df,
                                      input_features, output_feature="TX_FRAUD", scale=True):
        # By default, scales input data
        if scale:
            (train_df, test_df) = self.scaleData(train_df, test_df, input_features)
    ...
    def card_precision_top_k_custom(self, y_true, y_pred, top_k, transactions_df):
        # Let us create a predictions_df DataFrame, that contains all transactions matching the indices of the
        # current fold (indices of the y_true vector)
        ...
        nb_compromised_cards_per_day, card_precision_top_k_per_day_list, mean_card_precision_top_k = \
            self.card_precision_top_k(predictions_df, top_k)
    ...
    def execution_times_model_collection(self, fitted_models_and_predictions_dictionary):
    ...
    def get_class_from_fraud_probability(self, fraud_probabilities, threshold=0.5):
        predicted_classes = [0 if fraud_probability < threshold else 1
                             for fraud_probability in fraud_probabilities]

        return predicted_classes

    def threshold_based_metrics(self, fraud_probabilities, true_label, thresholds_list):
        results = []

        for threshold in thresholds_list:

            predicted_classes = self.get_class_from_fraud_probability(fraud_probabilities, threshold=threshold)
    ...
    def model_selection_performances(self, performances_df_dictionary,
        ...
        for model_class, performances_df in performances_df_dictionary.items():
            ...
            default_performances = performances_df[
                performances_df['Parameters summary'] == default_parameters_dictionary[model_class]]
            default_performances = default_performances.round(decimals=3)

            mean_performances_dictionary["Default parameters"].append(
                default_performances[performance_metric + " Test"].values[0])
            std_performances_dictionary["Default parameters"].append(
                default_performances[performance_metric + " Test Std"].values[0])

            # Get the performances for the best estimated parameters
            performances_summary = self.get_summary_performances(performances_df,
    ...
    def kfold_cv_with_classifier(self, classifier,
                                 X,
                                 y,
                                 n_splits=5,
    ...
    def get_tx_stats(self, transactions_df, start_date_df="2022-02-01"):
        # Number of transactions per day
        nb_tx_per_day = transactions_df.groupby(['TX_TIME_DAYS'])['CUSTOMER_ID'].count()
        # Number of fraudulent transactions per day
        nb_fraudulent_transactions_per_day = transactions_df.groupby(['TX_TIME_DAYS'])['TX_FRAUD'].sum()
        # Number of fraudulent cards per day
        nb_compromised_card_per_day = transactions_df[transactions_df['TX_FRAUD'] == 1].groupby(
    ...
    def get_template_tx_stats(self, ax, fs,
                              start_date_training,
                              title='',
                              delta_train=7,
                              delta_delay=7,
    ...
    def get_template_roc_curve(self, ax, title, fs, random=True):
    ...
    def get_template_pr_curve(self, ax, title, fs, baseline=0.5):
        ax.set_title(title, fontsize=fs)
        ax.set_xlim([-0.01, 1.01])
        ax.set_ylim([-0.01, 1.01])
    ...
    def get_performance_plot(self, performances_df,
        ...
        if summary_performances is not None:
            best_estimated_parameter = \
                summary_performances[performance_metric][['Best estimated parameters']].values[0]
            best_estimated_performance = float(
                summary_performances[performance_metric][['Validation performance']].values[0].split("+/-")[
                    0])
    ...
    # Get the performance plot for a single performance metric
    def get_execution_times_plot(self, performances_df,
                                 title="",
                                 parameter_name="Tree maximum depth"):
    ...
    def plot_decision_boundary_classifier(self, ax,
                                          classifier,
                                          train_df,
    ...
    def plot_decision_boundary(self, classifier_0,
                               train_df,
                               test_df):
        fig_decision_boundary, ax = plt.subplots(1, 3, figsize=(5 * 3, 5))

        self.plot_decision_boundary_classifier(ax[0], classifier_0,
                                               train_df,
                                               title="Decision surface of the decision tree\n With training data",
                                               plot_training_data=True)

        self.plot_decision_boundary_classifier(ax[1], classifier_0,
                                               train_df,
                                               title="Decision surface of the decision tree\n",
                                               plot_training_data=False)

        self.plot_decision_boundary_classifier(ax[2], classifier_0,
                                               test_df,
                                               title="Decision surface of the decision tree\n With test data",
                                               plot_training_data=True)
    ...
    def prepare_generators(self, training_set, valid_set, batch_size=64):
    ...
    def training_loop(self, model, training_generator, valid_generator, optimizer, criterion, max_epochs=100,
                      apply_early_stopping=True, patience=2, verbose=False):
    ...
    def per_sample_mse(self, model, generator):
        model.eval()
